...
if (check_entity_relationship($user->guid, 'claimed', $entity->guid)) {
	echo '<div class="hj-mechanics-uncover-badge">' . elgg_echo('hj:mechanics:alreadyclaimed') . '</div>';
	$has = true;
}
...
if ($points_required > 0) {
	...
	echo elgg_view_layout('hj/dynamic', array(
		'grid' => array(10, 2),
		'content' => array(elgg_echo('hj:mechanics:pointsrequired'), "$user_points / $points_required")
	));
...
if ($badges_required) {
	echo elgg_echo('hj:mechanics:badgesrequired') . '<br />';
...
if ($points_cost) {
	echo elgg_echo('hj:mechanics:badge:pointscost', array($points_cost));
...
if ($eligibility && !$has) {
	...
	echo elgg_echo('hj:mechanics:badge:congratulations') . '<br />';
	echo elgg_view('output/url', array(
		'text' => elgg_view('input/button', array('value' => elgg_echo('hj:mechanics:badge:claim'), 'class' => 'elgg-button-action')),
