...
function hj_mechanics_get_scoring_rules() {
	/**
	 * Scoring Rule Parameters
	 * 
	 * @param $handler_type event|hook
	 * @param $handler_name
	 * @param $entity_type
	 */
	return elgg_trigger_plugin_hook('hj:mechanics:scoring:rules', 'all', null, array());
}

function hj_mechanics_get_scoring_rules_list() {
...
function hj_mechanics_get_user_score($user, $time_lower = null, $time_upper = null) {
	if (!$user) {
...
function hj_mechanics_get_user_action_total($user, $rule, $time_lower = null, $time_upper = null) {
...
function hj_mechanics_get_user_recur_total($user, $rule, $time_lower = null, $time_upper = null) {
...
function hj_mechanics_get_badge_types() {
	...
	$return = elgg_trigger_plugin_hook('hj:mechanics:badge_types', 'object', null, $return);

	return $return;
}

function hj_mechanics_prepare_badge_relationship_tags() {
...
function hj_mechanics_check_user_eligibility_for_badge(ElggObject $badge, ElggUser $user) {
	...
	if ($user) {
		$user_points = hj_mechanics_get_user_score($user);
	} else {
		$user_points = 0;
	}
